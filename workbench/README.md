...
低消耗内核：小票与 MCP 默认返回绑定操作的上下文分段路径，每段不超过 12 KiB；完整读取一次后提交，普通软字数不反复计数找补。记忆幂等恢复、原文证据召回、累计构筑事实及离线 OpenCode 用量分析见[接口说明](../.agents/skills/story-bible-studio/references/low-cost-runtime.md)。升级后重启工作台与 MCP 连接以加载新工具；现有会话阈值和正文不自动修改。
...
首次安装、创建作品和第一段故事见[项目入门教程](../README.md)。以下按操作查阅。
...
- 构筑提供主题链接，选中历史讨论时链接包含该条讨论。前进后退可返回主题，窄屏也保留主题搜索。
...
- 「写回应」与「返回原文」可从当前阅读位置往返输入区。
- 「会话链接」与每轮的「定位链接」可收藏或在同一本地工作台打开；指定回合会补载较早内容。地址栏记录当前作品和会话，回合跳转后记录回合编号，支持浏览器前进后退。链接依赖本地服务地址，端口改变后需使用新的地址。
- 右侧摘要显示当前生效的字数、人称、扮演权限与状态栏；展开「调整字数与互动方式」编辑，也可设置双语。默认持续保存到本会话，项目默认只影响新会话，本轮覆盖随下一张小票交接并在摘要中单独标明。
...
- 构筑与会话阅读共用昼夜主题；新主题表单关闭后保留未提交草稿。浏览器存储失败时显示提醒，并在当前页面暂存输入；离开前应复制备份。
...
Luna 是 Codex 专用的辅助候选配置。OpenCode、Antigravity 等非 Codex 平台由当前主 Agent 完成记忆整理、章节检查和复杂召回，不启用 Luna，也不改写已有会话中的辅助设置；压缩阈值和核心提交照常执行。用户仍可显式把平台支持的 Luna 选作主模型，这与辅助配置独立。

### CLI 检测与任务运行

工作台使用本机平台账号，未指定模型时沿用平台默认配置，不接收 API 密钥。Windows 需要本机 `.exe` / `.com`，不执行 `.cmd` / `.ps1` 包装脚本。Codex 会检查 PATH、常用独立安装位置、桌面版的版本化 CLI 目录，以及 npm 包中的原生程序；桌面版升级后重新查找，无需保存版本目录路径。仍未找到时，可在启动终端设置 `STORY_STUDIO_CODEX` 或 `STORY_STUDIO_OPENCODE` 为可执行文件的绝对路径。不要把这些本机路径写进公开仓库。安装后需重启工作台以取得新的 PATH。
...
小票默认短指令，可切换「完整指令」。Agent 接手小票、加载规范与提交的约定统一见 [Agent 指南](agent-guide.md)。
...
```powershell
.\.venv-workbench\Scripts\python.exe -B workbench/opencode_server.py configure --url http://127.0.0.1:4096 --ask-password
.\.venv-workbench\Scripts\python.exe -B workbench/opencode_server.py check
```
...
运行根目录 `安装MCP.ps1`，或安装到入门教程已创建的项目虚拟环境：
...
管理写入在核心锁内核对版本和待提交事务。HTTP 只开放管理操作与固定平台的小票调度；`agent_start` 只接受已保存的小票路径、平台名、经过校验的可选模型 ID、Codex 推理强度和显式重试标记，不接受命令或正文提交。`agent_models` 仅读取固定平台的模型目录，返回模型 ID、名称、支持的推理强度和提示，不返回平台凭据。`agent_stop` 只接受任务 ID。写入接口复用来源检查与令牌验证；生成提交工具仍仅由 Agent 经 CLI/MCP 调用。不提供任意文件读写、任意 shell、删除会话、自动修复或远程监听接口。
