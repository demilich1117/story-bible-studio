...
- 阅读导航新增「写回应」与「返回原文」，可从当前阅读位置往返输入区。打开会话的默认定位仍是恢复上次阅读位置。
- 「会话链接」与每轮的「定位链接」可收藏或在同一本地工作台打开；指定回合会补载较早内容。地址栏记录当前作品和会话，回合跳转后记录回合编号，支持浏览器前进后退。链接依赖本地服务地址，端口改变后需使用新的地址。
- 右侧先展示当前生效的字数、人称、扮演权限与状态栏摘要；展开「调整字数与互动方式」再编辑。创作要求默认折叠，本轮覆盖在摘要中单独标明。
...
- 构筑提供主题链接，选中历史讨论时链接包含该条讨论。前进后退可返回主题，窄屏也保留主题搜索；搜索结果会展开对应模块，并明确显示无匹配结果。
...
工作台沿用本机平台的默认模型和账号，不接收 API 密钥。Windows 需要本机 `.exe` / `.com`，不执行 `.cmd` / `.ps1` 包装脚本。Codex 会检查 PATH、常用独立安装位置、桌面版的版本化 CLI 目录，以及 npm 包中的原生程序；桌面版升级后重新查找，无需保存版本目录路径。仍未找到时，可在启动终端设置 `STORY_STUDIO_CODEX` 或 `STORY_STUDIO_OPENCODE` 为可执行文件的绝对路径。不要把这些本机路径写进公开仓库。安装后需重启工作台以取得新的 PATH。
...
启动提示词只携带短小票、平台差异和接入指南路径，不叠加备用「完整指令」。`studio_ticket` 已完成准备或恢复，拿到结果后按 kind/status 处理，不再重复 prepare。指南只保留公共约束和正常提交入口；构筑、恢复及预算细节按需读取。MCP 的 `studio://guide` 与本地接入指南二选一，文学技能单独通过 `studio://skill` 或本地 SKILL.md 加载，同一任务不重复读取。新平台上下文仍需加载必需规范与当前创作材料，这部分不以精简提示词为由省略。
