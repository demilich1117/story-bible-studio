...
export class AgentPanel {
  constructor(api, scope, completed) {
    ...
    this.root.innerHTML=`<label class="field">生成方式<select id="agent-provider"><option value="manual">复制小票</option><option value="codex">Codex · 本地生成</option><option value="opencode">OpenCode · 本地生成</option><option value="opencode_server">OpenCode · 共享后台</option><option value="antigravity">Antigravity · 本地生成</option></select></label>
  ...
  async refreshProviders(){
    ...
    const labels={codex:'Codex · 本地生成',opencode:'OpenCode · 本地生成',opencode_server:'OpenCode · 共享后台',antigravity:'Antigravity · 本地生成'};
  ...
  selectedReasoning(){return this.select.value!=='manual'?(this.reasoningSelect.value||null):null;}
  refreshReasoning(){
    const provider=this.select.value,codex=provider==='codex',hint=this.root.querySelector('#agent-reasoning-hint');
    this.root.querySelector('#agent-reasoning-field').hidden=provider==='manual';hint.hidden=provider==='manual';
    ...
    const model=this.catalogs[provider]?.find(item=>item.id===this.selectedModel());
    const efforts=model?.reasoning_efforts||(codex?Object.keys(labels):provider==='antigravity'?['low','medium','high']:[]),saved=this.read(this.reasoningKey())||'';
    ...
    this.reasoningSelect.disabled=codex&&!!this.codexBackendOutdated;
    hint.textContent=this.invalidReasoning?'此模型不再支持上次的推理强度，请重新选择。':model?
      ...
      (provider.startsWith('opencode')?'选择目录中的模型后可查看可用强度；目录缺少能力信息时只能使用默认值。':provider==='antigravity'?'Antigravity 支持低、中、高三档；默认选项沿用平台设置。':'选择目录中的模型后可查看其支持的强度；当前选项是否可用由 CLI 确认。');
  }
  async refreshModels(){
    ...
    const populate=(models,value)=>{
      ...
      this.modelInput.value=value;this.modelInput.placeholder=provider==='codex'?'填写 Codex 模型 ID':provider==='antigravity'?'填写 agy models 中的模型 ID':'provider/model';
  ...
  hint(){const p=this.providers?.find(p=>p.id===this.select.value);const base=p&&!p.available?p.reason:p?.hint|| (this.select.value==='manual'?'复制小票后交给你选择的 Agent。':'使用本机平台账号，每次从当前任务的已保存上下文开始。');this.root.querySelector('#agent-hint').textContent=base+((this.select.value.startsWith('opencode')||this.select.value==='antigravity')?' 记忆整理和复杂召回由主 Agent 完成，不启用 Luna 辅助配置。':'');}
  ...
  render(){
    ...
    jobModel.textContent=job?`本次请求主模型：${job.model||'平台默认（未指定）'} · 推理强度：${job.reasoning_effort||'CLI 默认（未指定）'}${job.helper_policy==='main_agent_only'?' · 辅助整理：主 Agent':''}`:'';
  ...
  async send(ticket,retry=false){
    ...
    if(this.select.value!=='manual'&&this.invalidReasoning)throw Error('此模型不支持所选推理强度，请重新选择或使用默认值。');
    ...
    try{record.job=await this.api('agent_start',{ticket_path:ticket.ticket_path,provider,retry,model,reasoning_effort},true);}
