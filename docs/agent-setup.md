...
构筑与 RP 都可使用 [本地直接生成](../workbench/README.md#直接生成构筑与续玩共用)。检查所选平台的 CLI 路径和登录状态，再重启工作台。不要把桌面客户端已登录等同于 CLI 已登录；账号登录由用户在平台的正常流程完成，不索取或复制凭据。Windows 调度只运行原生可执行文件，不运行 shell 包装脚本。无需 MCP 也可使用项目 CLI 完成小票。

首次验证使用独立临时作品，核对准备、核心提交与页面回显。没有实际调用平台时，明确区分模拟流程验证和真实模型联通。直接生成不会自动接管已有桌面聊天窗口。
